...
;; Created: 12 Dec 1992
;; Version: advice.el,v 2.9 1994/02/21 08:03:39 hans Exp
...
;; 1994/02/21 08:03:39|2.9|~/packages/advice.el.Z|
...
;; or from one of the Emacs Lisp archive sites, or send email to
;; <hans@cs.buffalo.edu> and I'll mail it to you. Because future version of
;; this package will not support Emacs-18 anymore, you might want to check
;; any of these locations for `advice18.el' if you are looking for an 
;; implementation of Advice that runs under Emacs-18.

;; @ Overview, or how to read this file:
;; =====================================
;; NOTE: This documentation is slightly out of date. An up-to-date version
;; will soon be available as an info file (thanks to the kind help of Jack
;; Vinson and David M. Smith). Until then you can use `outline-mode' to help
;; you read this documentation (set `outline-regexp' to `";; @+"').
;;
...
;; - You cannot anymore use automatic activation of (forward) advice in a
;;   v18 Emacs if you use a v19 byte-compiler.

;; @ Known bug:
;; ============
;; - Using automatic activation of (forward) advice in a v19 Emacs will break
;;   the function `interactive-p' when it is used in the body of a `catch'
...
;; in your `load-path' (you can preload Advice "by hand" before you compile
...
;; @@ Preloading:
;; ==============
;; If you preload the complete Advice or its autoloads into a dumped Emacs
...
;; value after Advice got loaded. If it doesn't set it explicitly in
;; your .emacs with
;;
...
;; @@ Terminology:
;; ===============
;; - Emacs-19: FSF's version of Emacs with major version 19
...
;; Merged with version of Lemacs 19.9: Fixed infinite recursion bug in
;; jwz's adaption of `ad-docstring' with use of `ad-real-documentation'.
;; (ad-recover-all, ad-scan-byte-code-for-fsets): Removed unused
;; condition variable `ignore-errors'.
;;
...
;; (ad-execute-defadvices): Copy advice infos so that they
;; won't get allocated in unmodifiable pure space during preloading.
;;
;; Revision 2.2  1993/12/23  02:32:34  hans
;; (ad-start-advice-on-load): Default changed to t.
;;
;; Revision 2.1  1993/05/26  00:07:58  hans
;; Merged with FSF installation of version 2.0.
;; (defalias, define-function): Advised to properly handle automatic
;; advice activation in Emacs-19.7 and later.
;; (ad-preactivate-advice): Fix minor bug.
;;
...
;; Emacs-19/Lemacs cross-compatibility
;; (compiled-function-p is an obsolete function in Emacs-19):
...
(defun ad-subr-arglist (subr-name)
  ...
  (cond ((ad-subr-args-defined-p subr-name)
	...
	(t (let ((doc (if ad-emacs19-p
			  (ad-real-documentation subr-name t))))
	     (cond ((and doc
			 (string-match
			  "[\n\t ]*\narguments: ?\\((.*)\\)\n?\\'" doc))
		    (ad-define-subr-args
...
the advised function (this should only be used with a v19 byte-compiler).
`freeze' implies `activate' and `preactivate'.  The documentation of the
advised function can be dumped onto the `DOC' file during preloading.
...
;; Needed for Emacs-19 (in v18s and Lemacs this is just a noop):
...
`ad-defined-function' bound to the name of the function.  This advice was
mainly created to handle forward-advice for byte-compiled files created
by jwz's byte-compiler used in Emacs-19."
  (let ((ad-defined-function (ad-get-arg 0)))
...
;; Needed for Emacs-19 (seems to be an identical copy of `defalias',
...
(defadvice documentation (after ad-advised-docstring first disable preact)
  ...
  (if (and (stringp ad-return-value)
	...
	(cond ((ad-is-advised function)
	       ...
	       ;; Handle Emacs-19's optional `raw' argument: 
	       (if (not (ad-get-arg 1))
		   (setq ad-return-value
...
(defun ad-enable-definition-hooks ()
  ...
  (cond ((and ad-use-jwz-byte-compiler ad-emacs19-p)
	...
	(ad-use-jwz-byte-compiler
	 ;; disable definition hooks for v18 + jwz's compiler, because the
	 ;; redefinition of `byte-code' breaks `interactive-p':
	 (error
	  "Advice error: You cannot use definition hooks %s"
	  "in Emacs-18 with a v19 compiler."))))
