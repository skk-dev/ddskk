...
;; を目的とします。 AT 互換機用の日本語 106 キーボードは普通 JIS 配列の刻印が
;; ありますが、まずはこれに対応する予定です。PC-98 への対応はこれを少し変更す
;; ればできると思います。
;;
;;  -*- 問題点 -*-
;;
...
;; していましたが、仮名入力ではこれができません。そこで、"Q" を押すことで変換
;; 開始位置を指定する必要があります。 要は、abbrev モードと同様のやり方で変換
;; を開始することになります。例えば「春」の入力は
;;
;; `Q' はる ⇒ ▽はる [SPC] ⇒ ▼春
;;
;; または
...
;; すが、仮名入力ではこれも使えません。仮名入力できるキーボードでファンクショ
;; ンキーが使えないことはあまり無いだろうと思い、デフォルトではこれらをファン
;; クションキーに退避してみました。デフォルトでは以下のようになっています。ユー
;; ザオプションなので自由に変更できます。
;;
;; [f2]  … 変換開始点の指定
;; [f3]  … 接頭辞または接尾辞変換
...
;;
;; これらのキー定義は一時的なものです。よりよいキー定義を考えて修正する予定で
;; あり、ユーザのご意見に期待しています。
;;
;; {TODO}
;;
;; o 個別のキーボード配列への対応コードはそれぞれ別ファイルにして、モジュール
;;   的にロードすることにした。そこで、今後はできるだけモジュールを追加する。
