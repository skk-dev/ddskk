...
となります。ここで ``RET`` を打鍵することで「さいきてき」の辞書登録モードから抜け、
個人辞書に
...
DDSKK では、専用のインクリメンタル・サーチプログラムを Emacs 添付の ``isearch.el`` の
...
動すると、インクリメンタル・サーチを起動したバッファの入力モードと同一の入力モー
ドで、キーとなる文字の入力が可能となります。
...
チュートリアルは ``M-x skk-tutorial`` で、英語版チュートリアルは
...
skk-tut-file
  ...
  です。この場合であれば、 ``/usr/local/share/skk/SKK.tut.E`` になります。
...
skk-tut-lang
  ...
  この変数よりも ``C-u M-x skk-tutorial`` による言語指定が優先されます。
...
.. [#] ``M-y`` の ``isearch-yank-kill`` 、 ``M-p`` の ``isearch-ring-retreat`` 、
