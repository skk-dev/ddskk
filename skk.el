...
リストの各要素は、\(現在の状態@入力 次の状態 出力\) \(但し、\"@\" は連
接\) を意味する。
出力に指定できるものは、文字列、文字列を car, cdr に持つ dot pair、関
数名シンボルのいずれか。

dot pair は、カナモードのときは car の文字列、かなモードのときは cdr の
文字列が挿入される。
文字列のみ指定されている場合は、入力モードにかかわらずその文字が挿入さ
れる。
...
(defcustom skk-rom-kana-rule-list
  ...
  "*ローマ字かな変換のオートマトンの状態遷移規則で、ユーザーの追加の設定を行なうもの。
...
カタカナを変換により求めたいが、個辞書にはカタカナのみの候補を取り込みたくな
い、など、個人辞書が必要以上に膨れるのを抑える目的に使用できる。

個人辞書に取り込まない見出し語については補完が効かないので、注意すること。"
  :type 'function
  :group 'skk )
...
(defcustom skk-use-relation nil
  "*skk-relation を使用する。これは直前の変換を憶えておくことで、
変換効率を良くしようという試み。" 
  :type 'boolean
  :group 'skk )

(defcustom skk-use-rdbms nil
  "*Non-nil であれば、RDBMS を利用した変換を行なう。" 
  :type 'boolean
  :group 'skk )
...
(defvar skk-input-mode-string skk-hiragana-mode-string
...
上記のどちらのモードでもなければ abort-recursive-edit と同じ動作をする。"
  ...
  (skk-remove-minibuffer-setup-hook
   ...
   (function (lambda () (add-hook 'pre-command-hook 'skk-pre-command 'local))) )
...
dependent."
  ...
  (if (not skk-mode)
    ...
    (cond (skk-use-vip (skk-vip-mode))
	  (skk-use-viper (require 'skk-viper)) )
    ;; .skk で skk-kakutei-key の変更が可能になるように。
    ...
    (add-hook 'pre-command-hook 'skk-pre-command 'local)
    (make-local-hook 'post-command-hook)
    (add-hook 'post-command-hook 'skk-after-point-move 'local)
    (skk-j-mode-on)
...
(defun skk-regularize ()
  ...
  (and skk-use-rdbms (require 'skk-rdbms))
  ...
  (if skk-use-relation
      (progn
	(require 'skk-attr)
	(or (member '(skk-search-relation) skk-search-prog-list)
...
(defun skk-reread-private-jisyo (&optional quiet)
		...
		word-tail (skk-substring word (1- word-len)
          ...
          (if (not (and (string= midasi-tail word-tail)
            ...
            (if (not (string= (skk-substring midasi pos2 midasi-len)
              ...
              (setq new-skk-henkan-key (concat
					(skk-substring midasi 0 pos2)
					(skk-auto-okurigana-prefix okuri-first) ))
...
(defun skk-auto-okurigana-prefix (okurigana)
