...
;; Version: $Id: skk-comp.el,v 1.19 2001/09/15 03:35:15 czkmt Exp $
;; Keywords: japanese
;; Last Modified: $Date: 2001/09/15 03:35:15 $
...
トする。"
  ...
  (cond
   ((and skk-henkan-on
    ...
    (skk-comp-do (not (eq last-command
			  'skk-comp-do)))
  ...
  (let ((inhibit-quit t)
    ...
    (cond
     ...
     (t
      ...
      ;; 一時変数に移し変えておく。
      (when (setq c-word (or (let ((word (skk-comp-do-1
					  skk-comp-key
					  first)))
			       (if (member word
					   skk-comp-stack)
				   (skk-comp-do-1 skk-comp-key
						  first)
				 word))
			     ;;
